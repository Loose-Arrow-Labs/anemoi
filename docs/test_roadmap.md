...
| 28 inference forwarding gateway | `POST /v1/chat/completions` maps model field to domain, runs decide, forwards to selected runtime, streams response, and embeds the decision's structured explanation inline in gateway error responses (issue #174). | `anemoi-daemon`, `anemoi-runtime`, `anemoi-core` | Passing |
